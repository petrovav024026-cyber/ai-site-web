...
type Input = { N:number; complexity:"low"|"mid"|"high"; K:number; R:number; A:number };
type Result = { capex:number; opex:number; saving:number; roi:number; payback:number|null };

export default function Client(){
  const [inp, setInp] = useState<Input>({ N:3, complexity:"mid", K:10000, R:1000, A:40 });
  const [res, setRes] = useState<Result|null>(null);
  const [loading, setLoading] = useState(false);

  function upd<K extends keyof Input>(k:K, v:Input[K]){ setInp(s=>({...s,[k]:v})); }

  async function calc(){
    setLoading(true);
    try{
      // simple local calc to avoid server dependency in demo
      ...
      const roi = denom>0 ? (12*saving - denom)/denom : 0;
      let payback = None;
    }catch{}
  }
  ...
  async function calcLocal(){
    ...
    try{
      ...
      setRes({ capex: Math.round(capex), opex: Math.round(opex), saving: Math.round(saving), roi, payback });
    ...
    <div className="container">
      ...
      <div className="tok-layout">
        <section>
          ...
          <select value={inp.complexity} onChange={e=>upd("complexity", e.target.value as any)}>
        ...
        <aside className="tok-result">
          {!res? <p>Заполните параметры и нажмите «Рассчитать».</p> : (
            <div>
              ...
              <div style={{margin:"4px 0 12px"}}>Окупаемость: {res.payback===null? "не окупается" : res.payback+" мес"}</div>
              <button className="pill" onClick={()=>alert("Сохранить расчёт → КП (mock)")}>Сохранить расчёт → КП</button>
            </div>
          )}
        </aside>
      </div>
